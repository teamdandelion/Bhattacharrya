...
nk_S3 = 2 * inv(nk_iS1 + nk_iS2);

nk_det = det(nk_S1)^(-1/4) * det(nk_S2)^(-1/4)*det(nk_S3)^(1/2);
nk_a = -.25* nk_mu1 * nk_iS1 * nk_mu1';
nk_b = -.25* nk_mu2 * nk_iS2 * nk_mu2';
...
e_S1 = S + (G'*Kc(:,1:n1)  *Kc(1:n1,:)  *G)/n1;
e_S2 = S + (G'*Kc(:,1+n1:n)*Kc(n1+1:n,:)*G)/n2;

e_invS1=inv(e_S1);
e_invS2=inv(e_S2);

e_mu3 = (e_invS1 * e_mu1' + e_invS2 * e_mu2')'/2;
e_S3  = 2 * inv(e_invS1 + e_invS2);

e_det1 = det(e_S1)^(-1/4);
e_det2 = det(e_S2)^(-1/4);
e_det3 = det(e_S3)^(1/2);

e_exp1 = -e_mu1 * e_S1inv * e_mu1' /4;
e_exp2 = -e_mu2 * e_S2inv * e_mu2' /4;
